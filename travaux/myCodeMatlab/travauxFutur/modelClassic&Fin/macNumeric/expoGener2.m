...
%TTI 1 ms le service
%tt3 est utilisé dans le programme principale pour calculer la distribution
%de proba de chacune des files
...
OutpuVerfiAAAbbb= strcat('PoissonArrive-', num2str(n), '.dat');
fiTrame = fopen(OutpuVerfiAAAbbb, 'w');  %wt
...
while (j<=B) 
    ...
    Ar(n,i)=1; %randi([1, lambda(n)], 1, 1); %générer un nbre de clients en entrée
    fprintf(fiGener, '%d \t %d \t %f \t %d \n', i-1, j,  instantGener(n,i-1), Ar(n,i) );
    Arriv=Arriv+Ar(n,i);
    fprintf(fiTrame, '%d \t %d \t %f \t %d \t %g\n', i-1, j,  instantGener(n,i-1), Ar(n,i), Arriv );
...
%    
  ...
  %cumuler les arrivées durant TTI
    if (cpt==TTI)
      ...
      cumulArrive(n,j)=min(Buffer(n),Arriv);
      ...
      if (cpt>TTI)
        cumulArrive(n,j)=min(Buffer(n),Arriv-Ar(n,i));%Arriv-Ar(n,i);
        fprintf(fiArrivee, '%d \t %d  \t %d \n',   n, j, cumulArrive(n,j) );  %indice de la trame et l'arrivée pr cette tram
        Arriv=Ar(n,i);
        j=j+1;
        cpt=abs(cpt -TTI);%time(n,i-1);       
      else
           tt3(n)=tt3(n)+time(n,i-1); %+tsim;
      end  
