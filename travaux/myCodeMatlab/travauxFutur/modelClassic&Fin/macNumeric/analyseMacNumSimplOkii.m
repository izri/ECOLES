...
%Buffer=2000; %taille max du buffer
%lambda=6; 
...
c_max(2)=819; %410;
c_min(2)=8;
lambda(2)=625; %700; %320kb/s = 40 o/TTI //// 700 o/TTI = 5.6 Mb/s  375=3Mb/s
%625o/TTI=5Mbit/s; 650o/TTI=5.2Mbit/s ; 640o/TTI=5.12Mbit/s
%buffer(2)=500;
...
%       end
   ...
   if n>1  
       trame(n).file(cptFile).A =  trame(n-1).file(cptFile).X+ ArriveFile(cptFile,n);   %trame(n-1).file(cptFile).X+
%soit en rajout l état de la file lors de la construction de la trame (n61)
%au niveau des arrivées comme fait ci-dessus, ou bien on le rajout dans l
%état de la file (variable X) mais dans ce cas il faut changer les arrivées (retirer le X) et modifier les équations d évol de la variable aléa X lors du service

%        if(trame(n-1).file(cptFile).X >0 ) 
%            trame(n).file(cptFile).X=trame(n-1).file(cptFile).X;
%        else
%            trame(n).file(cptFile).X=0;
%        end
   else
...
%    disp(trame(n).file(cptFile).A );
%    disp('########"');
  ArrivInit(cptFile,n)= trame(n).file(cptFile).A;
    end
...
%          end
%    if n>1  
%        trame(n).file(h).A = trame(n-1).file(h).X+ ArriveFile(h,n);  
%    else
%        trame(n).file(h).A =  ArriveFile(h,n);  
%    end
  OutQUeue= strcat('etatQueue-', num2str(h), '.dat'); %pr les attentes et transmis
  
if n==1
     fiQueue = fopen(OutQUeue, 'w');  %wt
else
     fiQueue = fopen(OutQUeue, 'a+');  %wt
end
         if cycle==1
...
%            h
     ...
     %%%%%   fprintf('sortie = %g \n', sortiFile(h));
         AttenteFile{h}=[AttenteFile{h}, trame(n).file(h).X];
          %calcul des services pr la distrib de proba
         if (count >= TTI)
              if (nb(h)>0) 
               ni(h,nb(h))=ni(h,nb(h))+abs(TTI-tsim(h)); %service
              % disp(nb(h));
              % disp('***');
              % disp(sortiFile(h));
%              aaaa= sortiFile(h)
%               bbb= nb(h)
               nb(h)=max(0, nb(h)- sortiFile(h));
      %     tt1=nb(h)
        
         %  aaa=  trame(n).file(h).X 
       %     trameNB(n,innnn).clients=  ArrivInit(innnn,n)-sortiFile(innnn);
               %max(0, nb(h)- sortiFile(h)); %trame(n).file(h).out); %service
           %   tttt=trameNB(n,h).clients
              end
              tsim(h)=TTI;
              boolModif=1;
              stock(h)=hg; %pr garder une trame de l indice ou l on c arrêté
          else
              boolModif=0;
          end 
      ...
      %  trame(n).taille=min(sizeT, capacity);
        fprintf(fi, '%g \t %g \t %g \t %g \t %g \t %g \t %g \t %g \t %g \n', h, n ,  trame(n).file(h).out, trame(n).file(h).L, trame(n).file(h).X, sommeOUT,  trame(n).taille, ArrivInit(h,n), sortiFile(h));  %indice de la trame et sa taille
        fprintf(fiQueue, '%g \t %g \t %g \t %g \n', n , sizeT , ArrivInit(h,n), sortiFile(h));  %indice de la trame et sa taille
        fclose(fiQueue);
        h=h+1;        
    end %fin de la boucle qui parcours ttes les files
    
       for innnn=1:N
          trameNB(n,innnn).clients =  ArrivInit(innnn,n)-sortiFile(innnn); %nb(h);
          trameNB(n,innnn).transmis=sortiFile(innnn); 
       end
       trame(n).taille=min(sizeT, capacity);
    
    
       TailleTrame=[TailleTrame, trame(n).taille];
       fprintf(fiTrame, '%g \t %g \t %g \n',  n, trame(n).taille, (trame(n).taille*100)/C );  %indice de la trame et sa taille
...
for kkk=1:N
  ...
  % for uuu=1:H
     moyyyy=mean(AttenteFile{kkk}); %trame(uuu).file(kkk).X);   
     ecartTypA= std(AttenteFile{kkk}); %trame(uuu).file(kkk).X);
     tauxErrA=1.96; %intervalle de confiance de 95%
     ajustementIntervaA=(tauxErrA*ecartTypA)/sqrt(H); %moyyyy);
    for uuu=1:H
     fprintf(fiDistribbbbb, '%g \t %g \t %g  \t %g \n',  uuu , moyyyy, ajustementIntervaA, trame(uuu).file(kkk).X );  %indice de la trame et sa taille
    end    
...
ajustementInterva=(tauxErr*ecartTyp)/sqrt(H); %moy);
...
% fprintf('file %g \n', ll);
 tabC=[];
 PI{ll}.dist=zeros(1,buffer(ll)+1);
for ii=1:H
  %  tt=trameNB(ii,ll).clients;
    tabC=[tabC, trameNB(ii,ll).clients];
end
%tabC
%fprintf('%%%%%%%%%%%%%%%%%%%%%%%%%');
eletMin=min(tabC);
cumIndice=hist(tabC, 0:buffer(ll));
distttt=cumIndice/H;
id=1:length(distttt); %pr completer jusqu à buffer
PI{ll}.dist=distttt;
sum(PI{ll}.dist)
[val, idx]=find(PI{ll}.dist>0);
%PI{ll}.dist(idx);
end

% 23/07/2013
% for rr=1:N
%  Pi=ni(rr,:)/t3(rr); %T_max
%  sum(Pi);
%  PI{rr}.dist=[(1-sum(Pi)) Pi];
%  sum(PI{rr}.dist);
%  Pi=[];
% end

fprintf(fiTrameA, '%g \t %g \t %g \t %g \t %g \t %g \t %g \t %g \n', Buffer, moy, ecartTyp, ajustementInterva, intervaConf1, intervaConf2, PI{1}.dist(buffer(1)+1), PI{2}.dist(buffer(2)+1));  %indice de la trame et sa taille
fclose(fiTrameA);
...
for yy=1:N
    ...
    %   fprintf(fiDistrib, '%d \t %d \n',  xx,  PI{yy}(xx));  %indice de la trame et sa taille
       sommeProba=sommeProba+  PI{yy}.dist(xx); %pr vérifier que la distribution est bien = à 1
       fprintf(fiDistrib, '%d \t %f \t %f \n',  (xx-1),  PI{yy}.dist(xx), sommeProba);  %indice de la trame et sa taille
...
%pr calculer la distrib des taux de taux pr le remplissage des trames
for lal=1:N
% fprintf('file %g \n', ll);
 tabCA=[];
 PIA{lal}.dist=zeros(1,C+1);
for iai=1:H
  %  tt=trameNB(ii,ll).clients;
    tabCA=[tabCA, trameNB(iai,lal).transmis];
end
 
%fprintf('%%%%%%%%%%%%%%%%%%%%%%%%%');
eletMinA=min(tabCA);
cumIndiceA=hist(tabCA, 0:C);
disttttA=cumIndiceA/H;
idA=1:length(disttttA); %pr completer jusqu à buffer
PIA{lal}.distA=disttttA;
sum(PIA{lal}.distA)
[valA, idxA]=find(PIA{lal}.distA>0);
%PI{ll}.dist(idx);
end
...
for yya=1:N
    OutputFilee= strcat('distribOutFile-', num2str(yya), '.dat'); %pr les attentes et transmis
    fiDistribOut = fopen(OutputFilee, 'w');  %wt
    sommeProbaA=0;
    for xxa=1:C+1
    %   fprintf(fiDistrib, '%d \t %d \n',  xx,  PI{yy}(xx));  %indice de la trame et sa taille
       sommeProbaA=sommeProbaA +  PIA{yy}.distA(xxa); %pr vérifier que la distribution est bien = à 1
       fprintf(fiDistribOut, '%d \t %f \t %f \n',  (xxa-1),  PIA{yya}.dist(xxa), sommeProbaA);  %indice de la trame et sa taille
    end
  fclose(fiDistribOut);
end
