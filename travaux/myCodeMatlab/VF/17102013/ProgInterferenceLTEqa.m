...
varianteALGO=2; %1: tous les users utilisent la meme puissance de transmission dans tout le reseau; 2: les users d'une meme cellule sont sur la meme puisssance; 3: la puissance de transmission d un user est en fct de sa distance par rapport au eNB
varianteChooseRB=1;
...
%             end
       ...
       fprintf(fiSINRDebit, '%d  \t %d \t \t %d %d \t %f \t %f \t %f \n', t,xxC, COUNT,  indexRB, cellule{xxC,ta}.RB(indexRB).dist, cellule{xxC,t}.RB(indexRB).SINR, cellule{xxC,t}.RB(indexRB).Throughput);      
       end
        fclose(fiSINRDebit);
      end
