...
charge=0.7;
Nstabilisation=4; %pr verifier que pendant ce nbre d itérations les paramètres n ont pas changé
...
for indiCell=1:V
 ...
 %  fprintf('--------------- t=%g\t\t q=%g\t\t RB=%g \t \t SINR =%f  \n',t,indiCell, rb_aA, cell{indiCell,t}.RB(rb_aA).SINR);
      %30/08
      cell{indiCell,t}.RB(rb_aA).Throughput = W * abs(log(1 + cell{indiCell,t}.RB(rb_aA).SINR));
      cell{indiCell,1}.cumulDebit=cell{indiCell,1}.cumulDebit+cell{indiCell,t}.RB(rb_aA).Throughput; %20/08            
      Mat(t).qValue(indiCell,rb_aA)=W*abs(log(1+cell{indiCell,t}.RB(rb_aA).SINR)); %fct QValue 
      ...
      % if (length(intersect(T{sauvegardddd}.cellO{indiCell},rb_aA )) > 0)  
         ...
         diffGainTotal = Mat(t).qValue(indiCell,rb_aA); % - Mat(sauvegardddd).gainTotal(indiCell,rb_aA);
    ...
    %%%%%%%%%%%%%%%%
      ...
      for rbConfli=1:length(RB_common)     
         wwRbConfli=RB_common(rbConfli);%num de RB
            ...
            for ttaa=1:length(cell{q,ta}.voisins)
                 voisinn=cell{q,ta}.voisins(ttaa);
                 okuRBa =  intersect(cell{voisinn,ta}.rbUserPris, wwRbConfli) ; %indexe de l user utilisant le wwRbConfli
                 if (length(okuRBa)==1)
                  sommeDistUsers=sommeDistUsers + cell{voisinn,ta}.RB(okuRBa).puissance * ((1/ cell{voisinn,ta}.RB(okuRBa).dist)^beta); %on peut egalement passer par l user
                 %premiere etape: calculer les SINR     
                end                     
%                  if (length(RB_commonVoisin{voisinn}) >=1)  
%                 for f=1:length(RB_commonVoisin{voisinn}) %calcul de la plus grande partie du dénominateur pr le caclul du SINR
%                 ww=RB_commonVoisin{voisinn}(f);%num de RB
...
%              if (ww==wwRbConfli)   sommeDistUsers=sommeDistUsers + cell{voisinn,ta}.RB(ww).puissance * ((1/ cell{voisinn,ta}.RB(ww).dist)^beta); %on peut egalement passer par l user
%                 %premiere etape: calculer les SINR
%              end
%                 end
%               end  
      ...
      %30/08
                     ...
                     cell{q,t}.RB(wwRbConfli).Throughput = W *abs(log(1 + cell{q,t}.RB(wwRbConfli).SINR));
                    ...
                    Mat(t).qValue(q,wwRbConfli)=W*abs(log(1+cell{q,t}.RB(wwRbConfli).SINR)); %fct QValue 
     ...
     if(t>1)   
                      if (length(intersect(T{sauv}.Cel{q},wwRbConfli )) > 0)    
                       ...
                       %  disp('???????????????');  
                       Mat(t).gainTotal(q,wwRbConfli) = Mat(t-1).gainTotal(q,wwRbConfli)  +Mat(sauv).gainTotal(q,wwRbConfli) + Mat(t).gain(q,wwRbConfli);
                     ...
                     %05/09 a voir si c ca la fct à utuliser
                       %06/08/2013                   
                      else
                       %   disp('???????????????'); 
           ...
           diffGainTotal = Mat(t).gain(q,wwRbConfli)- Mat(t-1).gain(q,wwRbConfli); 
           diffGainTotal = Mat(t).qValue(q,wwRbConfli)- Mat(t-1).qValue(q,wwRbConfli); 
                                  
              %%%               fprintf(2, ' !!!!!!!!!!!!!!! %d \t \t %g \t %g \t %g \t %g \n', t,   Mat(t-1).gainTotal(q,wwRbConfli), Mat(t).gain(q,wwRbConfli),Mat(t-1).gain(q,wwRbConfli), Mat(t).gainTotal(q,wwRbConfli));
                    ...
                    Mat(t).gainTotal(q,wwRbConfli) = Mat(t).gain(q,wwRbConfli);
                    Mat(t).gainCell(q)=0;
                    Mat(t).sinrCell(q)=0;
                    ...
                    diffGainTotal = Mat(t).qValue(q,wwRbConfli);% - Mat(t-1).gainTotal(q,wwRbConfli);
               ...
               %25/09
               %disp(Mat(t).gainTotal(q,wwRbConfli));
if (t>1)            
    eee=  Mat(t).gainTotal(q,wwRbConfli) - Mat(t-1).gainTotal(q,wwRbConfli);
   % fprintf('q= %d \t eee= %d \n',q, eee);
end     
          ...
          if ( length(RB_usedOK)>1)
              ...
              for rbDISPO=1:length(RB_usedOK)
               ...
               %modif pr le calcul du SINR
                  cell{q,t}.RB(rbChoix).SINR = calculSINR(cell{q,ta}.RB(rbChoix).dist, sommeDistUsersA , PuisUsed(q,rbChoix), beta, noise);
                  cell{q,t}.RB(rbChoix).Throughput = W *abs( log(1 + cell{q,t}.RB(rbChoix).SINR));
            ...
            if ( length(RB_usedOK)==1)
                ...
                for ttaaea=1:length(cell{q,ta}.voisins)  
                    ...
                    if (length(okuRB)==1)
                        sommeDistUsersAb=sommeDistUsersAb + cell{voiizz,ta}.RB(okuRB).puissance * ((1/ cell{voizz,ta}.RB(okuRB).dist)^beta); %on peut egalement passer par l user
                            %premiere etape: calculer les SINR
                
                    end  
                end
                %modif pr le calcul du SINR  
                cell{q,t}.RB(rbChoix).SINR = calculSINR(cell{q,ta}.RB(rbChoix).dist, sommeDistUsersAb , PuisUsed(q,rbChoix), beta, noise);
                cell{q,t}.RB(rbChoix).Throughput = W *abs(log(1 + cell{q,t}.RB(rbChoix).SINR));
                if ((cell{q,t}.RB(rbChoix).SINR - cell{q,t}.RB(wwRbConfli).SINR) > 0)   
               ...
               if ( length(RB_usedOK)==0)
                  ...
                  if (newRB ~= wwRbConfli)
                        ...
                        %    fprintf(2,'Je change %g \t avec \t %g \t vois %g \n ',wwRbConfli,newRB, voisin);
                         ...
                         % cell{q,t}.RB(newRB).Throughput = W * abs(log(1 + cell{q,t}.RB(newRB).SINR));
                          
                            
                    if (okchoixRBdispo==1 ) %(cell{q,t}.RB(newRB).SINR - cell{q,t}.RB(wwRbConfli).SINR) >= epsilon)   
                           ...
                           % récupérer le num du user uRB ??????
                            ...
                            Mat(t).qValue(q,newRB)=W*abs(log(1+cell{q,t}.RB(newRB).SINR)); %fct QValue
...
%                             end
                           ...
                           % fprintf ('*******  JE SUIS ICI ******** \n');
                            ...
                            Mat(t).gainTotal(q,wwRbConfli)= 0; %25/09
%                             %%%%%%%%%%%%%%      affectation SINR     %%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                       %25/09       
%    sommeDistUsersAa=0;
%    for ttaaaZ=1:length(cell{q,ta}.voisins) 
%         voiz=cell{q,ta}.voisins(ttaaaZ);
%         okuRBe =  intersect(cell{voiz,ta}.rbUserPris, newRB) ; %indexe de l user utilisant le wwRbConfli
%          if (length(okuRBe)==1)
%            sommeDistUsersAa=sommeDistUsersAa + cell{voiz,ta}.RB(okuRBe).puissance * ((1/ cell{voiz,ta}.RB(okuRBe).dist)^beta); %on peut egalement passer par l user
%                 %premiere etape: calculer les SINR
%       
%          end  
%     end
        %modif pr le calcul du SINR      
 %%25/09                           cell{q,t}.RB(newRB).SINR = calculSINR(cell{q,ta}.RB(newRB).dist, sommeDistUsersAa, PuisUsed(q,newRB), beta, noise);
                        % %  fprintf('%f \n', cell{q,t}.RB(newRB).SINR);
                            %10/07/2013: rajout de la ligne permettant de calculer le
                            %débit avec la formule de shannon
      ...
      %30/08
                            ...
                            fprintf(fiSINR_DebitA, '%d \t %d \t %d \t %d \t %f \t %f \t %f \n', t,q,  voisin, newRB,cell{q,ta}.RB(newRB).dist, cell{q,t}.RB(newRB).SINR, cell{q,t}.RB(newRB).Throughput);  
                               %                                 '%d \t %d \t %d \t %d \t %f \t %f \t %f \n', t,q, voisin,  wwRbConfli, cell{q,ta}.RB(wwRbConfli).dist, cell{q,t}.RB(wwRbConfli).SINR, cell{q,t}.RB(wwRbConfli).Throughput);
                            %calcul de la fonction d'apprentissage 
                            Mat(t).qValue(q,newRB)=W*abs(log(1+cell{q,t}.RB(newRB).SINR)); %fct QValue
                            Mat(t).apprentissageRB(q,newRB)=cell{q,ta}.probCollision * Mat(t).qValue(q,newRB); %fct d'apprentissage
                            ...
                            if(t>1) 
                               ...
                               if (length(intersect(T{sauv}.Cel{q},newRB )) > 0)    
                                 ...
                                 %  disp('???????????????');  
                                  Mat(t).gainTotal(q,newRB) =Mat(t-1).gainTotal(q,newRB) + Mat(sauv).gainTotal(q,newRB) + Mat(t).gain(q,newRB);
                               ...
                               % Mat(t).gainCell(q)=Mat(t).gainCell(q)- Mat(t).gainTotal(q,wwRbConfli) + Mat(t).gainTotal(q,newRB);
                                ...
                                diffGainTotal = Mat(t).qValue(q,newRB); % - Mat(t).qValue(q,wwRbConfli);
                             
                              %  diffGainTotal = Mat(t).gainTotal(q,newRB) - Mat(t).gainTotal(q,wwRbConfli);
                 %%   fprintf(2, ' ----------- %d \t \t %g \t %g \t %g \t %g \n', t,   Mat(t-1).gainTotal(q,newRB), Mat(t).gain(q,newRB),Mat(t-1).gain(q,newRB), Mat(t).gainTotal(q,newRB));
                             ...
                             %   diffGainTotal = Mat(t).gainTotal(q,newRB);% - Mat(t-1).gainTotal(q,wwRbConfli);
                               ...
                               diffGainTotal = Mat(t).qValue(q,newRB);% - Mat(t-1).gainTotal(q,wwRbConfli);
...
%[  , ind1, ind2]  %j ai enlevé les indices 30/08 car j ai changé la fct
%intersect par la fct setdiff pr récupérer l'ensemble des rb qui n ont pas
%été tiré en commun avecles voisins
...
if (length(RB_Cellll) > 0)
  ...
  for uuRB=1:length(RB_Cellll)
      rb_A=RB_Cellll(uuRB);         
       ...
       for ttapa=1:length(cell{q,ta}.voisins)
                 ...
                 if (length(okuRBa)==1)
                  sumDit=sumDit + cell{voisinns,ta}.RB(okuRBa).puissance * ((1/ cell{voisinns,ta}.RB(okuRBa).dist)^beta); %on peut egalement passer par l user
                 %premiere etape: calculer les SINR     
                end  
 ...
 % 30/08 
      cell{q,t}.RB(rb_A).Throughput = W * abs(log(1 + cell{q,t}.RB(rb_A).SINR));
      cell{q,1}.cumulDebit=cell{q,1}.cumulDebit+cell{q,t}.RB(rb_A).Throughput; %20/08            
      Mat(t).qValue(q,rb_A)=W*abs(log(1+cell{q,t}.RB(rb_A).SINR)); %fct QValue 
      ...
      if (t>1)
       ...
       if (length(intersect(T{sauvegardddd}.cellO{q},rb_A )) > 0)  
         ...
         %cell{q,t}.probCollision*apprentissageRB(q,ww);
         %  disp('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');  
         Mat(t).gainTotal(q,rb_A) = Mat(t-1).gainTotal(q,rb_A) +Mat(sauvegardddd).gainTotal(q,rb_A) + Mat(t).gain(q,rb_A); %29/08   Mat(t-1).gainTotal
  ...
  %%  fprintf(2, ' ############## %d \t \t %g \t %g \t %g \t %g \n', t,   Mat(t-1).gainTotal(q,rb_A), Mat(t).gain(q,rb_A),Mat(t-1).gain(q,rb_A), Mat(t).gainTotal(q,rb_A));
 ...
 %  fprintf('--------------- t=%g\t\t q=%g\t\t RB=%g \t \t SINR =%f  \n',t,q, rb_aaA, cell{q,t}.RB(rb_aaA).SINR);
     %30/08 
      cell{q,t}.RB(rb_aaA).Throughput = W * abs(log(1 + cell{q,t}.RB(rb_aaA).SINR));
      cell{q,1}.cumulDebit=cell{q,1}.cumulDebit+cell{q,t}.RB(rb_aaA).Throughput; %20/08            
      Mat(t).qValue(q,rb_aaA)=W*abs(log(1+cell{q,t}.RB(rb_aaA).SINR)); %fct QValue 
...
%        else
       ...
       if (length(intersect(T{sauvegardddd}.cellO{q},rb_aaA )) > 0)  
         ...
         %cell{q,t}.probCollision*apprentissageRB(q,ww);
         %  disp('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');  
         Mat(t).gainTotal(q,rb_aaA) = Mat(t-1).gainTotal(q,rb_aaA) +  Mat(sauvegardddd).gainTotal(q,rb_aaA) + Mat(t).gain(q,rb_aaA);
         diffGainTotal = Mat(t).gainTotal(q,rb_aaA) - Mat(sauvegardddd).gainTotal(q,rb_aaA);
         %06/08/2013
       else
...
for allRB=1:length(cell{q,ta}.rbUserPris)
    ...
    gainTotalCell(q,rbActuel)= Mat(t-1).gain(q,rbActuel)+ Mat(t).gain(q,rbActuel);
   %% fprintf(2, ' %d \t \t %g \t %g \t %g \t %g \n', t,   Mat(t-1).gainTotal(q,rbActuel), Mat(t).gain(q,rbActuel),Mat(t-1).gain(q,rbActuel), Mat(t).gainTotal(q,rbActuel));
end
else
   
for allRB=1:length(cell{q,ta}.rbUserPris)
    rbActuel=cell{q,ta}.rbUserPris(allRB);
    gainTotalCell(q,rbActuel)=  Mat(t).gain(q,rbActuel);
   %% fprintf(2, ' %d \t \t %g \t %g \t %g \t %g \n', t,   Mat(t-1).gainTotal(q,rbActuel), Mat(t).gain(q,rbActuel),Mat(t-1).gain(q,rbActuel), Mat(t).gainTotal(q,rbActuel));
end 
end
...
fprintf(fiTestAAA, '%d \t %d \t %f \t %f \t %f \t %f \t %f \n', q, t, Mat(t).gainCell(q),Mat(t).gainCellAll(q), cell{q,ta}.probCollision, Mat(t).apprentiCell(q), Mat(t).sinrCell(q) );  
