   ...
   capacity=C;    
     ...
     while ( (h<=N) && (sizeT < capacity) ) %on vérifie aussi par rapport à la taille de la trame
       ...
       if cycle==1
         ...
         %   trame(n).file(h).A =  ArriveFile(h,n); %23/09
...
%else
  ...
  %   tailleRetirStep1(h)=0;
    %normal
   % a= c_max(h)-trame(n).file(h).out
   %b=capacity-trame(n).taille
  
          trame(n).file(h).L = min(max(0,c_max(h)-trame(n).file(h).out ), trame(n).file(h).X + max(0, trame(n).file(h).A-trame(n).file(h).out));
        %  disp(trame(n).file(h).L);
        %  sommeOUT
          trame(n).file(h).out = min(max(0, capacity-trame(n).taille)+sommeOUT, trame(n).file(h).L); %transmis
         
         % disp(trame(n).file(h).out);
          sommeOUT = min(sommeOUT+trame(n).file(h).out, max(0,capacity-trame(n).taille));
          %tt ce que j ai pu extraire jusqu à présent
         % sommeOUT2=sommeOUT
          % disp( trame(n).file(h).out);
          ...
          if (count >= TTI)
              if (nb(h)>0) 
               ...
               nb(h)=max(0, nb(h)- sortiFile(h)); %trame(n).file(h).out); %service
              end
              tsim(h)=TTI;
              boolModif=1;
