...
dur=10;
t_RNTP=10;%200; %a chaque 200 ms un msg RNTP est envoye sur le lien X2
dur1RNTP=t_RNTP;
                         ...
                         if(length(RB_commonNew)>0)
                          ...
                          %  ss=indexCell2(f); %num du user   
                           ...
                           %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                            ...
                            gainTotalCell(q,newRB)=Mat(t).gainTotal(q,wwRbConfli);
               ...
               sumDit=0;
                if (length(RB_Cellll) > 0)
                 ...
                 %  fprintf('** sauv=%g\t\t q=%g\t\t wwRbConfli=%g \t \t voisin= %g\n',sauv,q,rb_A, voisin);
                   ...
                   if t>1
                    Mat(t).gain(q,rb_A)= Mat(t).apprentissageRB(q,rb_A) - Mat(t-1).apprentissageRB(q,rb_A); %cell{q,t}.probCollision*apprentissageRB(q,ww);
                  %  disp('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');  
...
% % 06/08/2013
%    if t>1     
%       for cc=1:length(cellInterfOK) %V
%           cellTest=cellInterfOK(cc)
% %           Mat(t).gainCell(cellTest)
% %           Mat(t-1).gainCell(cellTest)
%       
%             diffG=Mat(t).gainCell(cellTest)-Mat(t-1).gainCell(cellTest);
%         
%          if (diffG==0)
%              okStable=okStable+1;
%              continuer=0;
...
%    
   ...
   if (continuer == 0)
                fprintf ('---------  FIN CONVERGENCE OK   --------- reçu %d  \t msg RNTP -- dur %d \t \n', cpt_RNTP, tConverg);
                break; 
   else
       tConverg=tConverg+1;
   end
