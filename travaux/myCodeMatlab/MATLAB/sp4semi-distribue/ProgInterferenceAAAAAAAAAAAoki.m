...
charge=0.7;
Nstabilisation=10; %pr verifier que pendant ce nbre d itérations les paramètres n ont pas changé
    ...
    %04/07/2013 g fait sortir ce test, car au cas o il faut
        ...
        % disp(RB_usedOK);
          if ( length(RB_usedOK)>1)
                maxRB=RB_usedOK(1);
               % fprintf(2, 'maxRB=%d \n', maxRB);
              for rbDISPO=2:length(RB_usedOK)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   for ttaaaZ=1:length(cell{q,ta}.voisins) 
        ...
        %modif pr le calcul du SINR      
                            cell{q,t}.RB(newRB).SINR = calculSINR(cell{q,ta}.RB(newRB).dist, sommeDistUsersAa, PuisUsed(q,newRB), beta, noise);
                        % %  fprintf('%f \n', cell{q,t}.RB(newRB).SINR);
                            %10/07/2013: rajout de la ligne permettant de calculer le
                            %débit avec la formule de shannon
