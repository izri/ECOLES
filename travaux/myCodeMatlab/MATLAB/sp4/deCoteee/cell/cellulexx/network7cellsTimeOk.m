...
%OutputFileThrough= strcat('fileThroughput.txt'); %pr le débit
%fi = fopen(OutputFileThrough, 'w');  %wt
...
% modified 05/06/2013 ==> integration de la puissance de chaque user
            ...
            diffGainTotal=0;
              for rbConfli=1:length(RB_common)
                ...
                fprintf(fiSINR_Debit, '%d \t %d \t %d \t %d \t %f \t %f \n', t,q, voisin,  wwRbConfli, cell{q,t}.RB(wwRbConfli).SINR, cell{q,t}.RB(wwRbConfli).Throughput); %t \t cell{q,t}.RB(wwRbConfli).SINR
                      ...
                      % newRB
                        ...
                        if (newRB ~= wwRbConfli)
                           ...
                           % récupérer le num du user uRB ??????
                            ...
                            if(t>1) 
                                Mat(t).gain(q,newRB)= Mat(t).apprentissageRB(q,newRB) - Mat(t-1).apprentissageRB(q,newRB); 
                                Mat(t).gainTotal(q,newRB) = Mat(t).gainTotal(q,newRB) + Mat(t).gain(q,newRB);
                               %%%%%%% 
                                diffGainTotal = Mat(t).gainTotal(q,newRB) - Mat(t-1).gainTotal(q,newRB);
