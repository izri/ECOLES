...
% %    %les même RB qu'elle
     ...
     for tt=1:length(cell{q,1}.voisins) 
        ...
        if voisin~=q %tt
            ...
            [RB_common, indexCell1, indexCell2]= intersect(cell{q,ta}.rbUserPris, cell{voisin,ta}.rbUserPris);
...
% modified 05/06/2013 ==> integration de la puissance de chaque user
                sommeDistUsers=sommeDistUsers + cell{voisin,1}.RB(ww).puissance * (1/ cell{voisin,ta}.RB(ww).dist)^beta; %on peut egalement passer par l user
            ...
            diffGainTotal=0;
              for rbConfli=1:length(RB_common)
                ...
                %%%%%%%%%%%%%%      affectation SINR     %%%%%%%%%%%%
                cell{q,ta}.RB(wwRbConfli).SINR = calculSINR(cell{q,ta}.RB(wwRbConfli).dist, sommeDistUsers, PuisUsed(q,wwRbConfli), beta, noise);
                %calcul de la fonction d'apprentissage 
                Mat(t).qValue(q,wwRbConfli)=log(1+cell{q,ta}.RB(wwRbConfli).SINR); %fct QValue
                ...
                if ( diffGainTotal < 0 )
                        ...
                        newRB=modifRB(q,wwRbConfli, cell{q,1}.nbrUser, cell{q,1}.rbUserPris  , Mat(t).tabRB ,N);
                        newRB
                        %04/07/2013 ==> a voir
                        if (newRB ~= wwRbConfli)
