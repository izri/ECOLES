...
for pop=1:V
    ...
    fiDistribA=load(nameFile); 
    [nLigne,kColonne]=size(fiDistribA);
    if ((nLigne>0) && (kColonne>0))
    plot(fiDistribA(:,2),fiDistribA(:,3)); %,'-+','color','red','LineWidth',2.0);
    %
...
for pp=1:V
   ...
   % fprintf(fiTest, '%d \t %d \t %f \t %f \t  \n', q, t, Mat(t).gainCell(q), cell{q,ta}.probCollision );  
    ...
    if ((nLigne>0) && (kColonne>0))
    plot(fiDistrib(:,2),fiDistrib(:,3),'red'); %,'-+','color','red','LineWidth',2.0);
    %
