...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
N = 15;     %Number of RBs/cell  ==> 6 pr SOAPS
R = 15;     %Radius of Hexagon==> rayon
charge=0.8;
Nstabilisation=50; %pr verifier que pendant ce nbre d itérations les paramètres n ont pas changé
             ...
             switch varianteChooseRB              
              ...
              for rbDISPO=1:length(RB_usedOK)
               ...
               %modif pr le calcul du SINR
                  ...
                  deb = W *abs( log(1 + sinr)); %cell{q,t}.RB(rbChoix).SINR)
                  qval=W*abs(log(1+sinr)); %fct QValue  %cell{q,t}.RB(rbChoix).SINR)
              ...
              %     if ((cell{q,t}.RB(rbChoix).SINR - cell{q,t}.RB(wwRbConfli).SINR) > 0)   
                   ...
                   if (appr- Mat(t).apprentissageRB(q,wwRbConfli) >0)
                   ...
                   while verif ==0   
                    ...
                    if nbre>0
                     ...
                     for ttaaea=1:length(cell{q,ta}.voisins)  
                      ...
                      debMax = W *abs(log(1 + sinrMax)); % cell{q,t}.RB(rbChoix).SINR)
                      qvalMax=W*abs(log(1+sinrMax)); %fct QValue  %cell{q,t}.RB(rbChoix).SINR)
                      apprMax=cell{q,ta}.probCollision * qvalMax; %fct d'apprentissage
                  
                      if (apprMax- Mat(t).apprentissageRB(q,wwRbConfli) >0)
              ...
              %    fprintf('PAS DE RB DISPO PR LA CELLULE %d \n', q);
                  ...
                  if (newRB ~= wwRbConfli)
                    ...
                    if (okchoixRBdispo == 1 ) %(cell{q,t}.RB(newRB).SINR - cell{q,t}.RB(wwRbConfli).SINR) >= epsilon)   
                        ...
                        cell{q,t}.RB(newRB).Throughput = W *abs(log(1 + cell{q,t}.RB(newRB).SINR)); % cell{q,t}.RB(rbChoix).SINR)
                        Mat(t).qValue(q,newRB)=W*abs(log(1+cell{q,t}.RB(newRB).SINR)); %fct QValue  %cell{q,t}.RB(rbChoix).SINR)
                 ...
                 %30/08 
                  ...
                  Mat(t).apprentissageRB(q,rb_A)=cell{q,ta}.probCollision * Mat(t).qValue(q,rb_A); %fct d'apprentissage
                    ...
                    %% fprintf(2, ' ******************** %d \t \t %g \t %g \t %g \t %g \n', t,   Mat(t-1).gainTotal(q,rb_aaA), Mat(t).gain(q,rb_aaA),Mat(t-1).gain(q,rb_aaA), Mat(t).gainTotal(q,rb_aaA));
                        ...
                        diffGainTotal(rb_A) = Mat(t).qValue(q,rb_A)- Mat(t-1).qValue(q,rb_A); 
