            ...
            if (length(RB_common) > 0)
              ...
              for rbConfli=1:length(RB_common) 
               ...
               %   Mat(t).gainTotal(q,wwRbConfli)= 0; % erreur corrige le 02/07/2013; il faut ini cette variable
                  ...
                  fprintf(fiSINR_Debit, '%d \t %d \t %d \t %d \t %f \t %f \t %f \n', t,q, voisin,  wwRbConfli, cell{q,ta}.RB(wwRbConfli).dist, cell{q,t}.RB(wwRbConfli).SINR, cell{q,t}.RB(wwRbConfli).Throughput); %t \t cell{q,t}.RB(wwRbConfli).SIN
                  ...
                  if(t>1) 
                           ...
                           %disp('MMMMMMMMMMMMMMMMMMmm');
                           fprintf(2, '%d \t \t %g \t %g \t %g \n', t, Mat(t-1).gainTotal(q,wwRbConfli), Mat(t).gain(q,wwRbConfli), Mat(t).gainTotal(q,wwRbConfli));
                        ...
                        %%%%%%% 
                        diffGainTotal = Mat(t).gain(q,wwRbConfli);
                        diffGainTotal = Mat(t).qValue(q,wwRbConfli);
                  end
...
%                             end
                        ...
                        %    Mat(t).gainTotal(q,newRB)= 0;    
                            ...
                            if(t>1) 
                               ...
                               fprintf('******² %d \t \t %g \t %g \t %g \n', t,   Mat(t-1).gainTotal(q,newRB), Mat(t).gain(q,newRB), Mat(t).gainTotal(q,newRB));
