...
%17/09
%modif au niveau des affectations pr chaque cellule le lnbre de rb dispo
%est différents du nbre de users
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
N = 12;     %Number of RBs/cell  ==> 6 pr SOAPS
...
% %     end 
    ...
    while (1)
     cell{j,1}.nbrRB=randi([1, NN], 1, 1); %affecter un certain nbr de RB par cellule
     if (cell{j,1}.nbrRB>1)
        break;
     end
    end
    while(1)
        ...
        if ( cell{j,1}.nbrRB > cell{j,1}.nbrUser)  %ca peut être égale mais le problm qui se pose lorsque c identique c que le principe de l algo pr changer de rb n est plus valable car il n y aura pas de rb dispo
             break;
        end
    end
    
    
    fprintf(fi, '%d \t %d \t %d  \n',  j, cell{j,1}.nbrUser, cell{j,1}.nbrRB);  
...
for k=1:V %etape init de mon algo
    ...
    % pris=[];
     ...
     for h=1:cell{k,1}.nbrUser
       ...
       while test ==0
       varT=  cell{k,1}.nbrRB;
       selectRB=randi([1, varT], 1, 1);  %selectionner un RB parmis N pr chaque user 
 ...
 %%            plot(cell{k,1}.RB(selectRB).x, cell{k,1}.RB(selectRB).y, 'ko');%05/09
       % end
         cell{k,1}.userRBPris=[cell{k,1}.userRBPris, h];
 ...
 % disp(cell{k,1}.userRBPris);   %17/09
end
...
% modified 05/06/2013 ==> integration de la puissance de chaque user
      ...
      %30/08
               ...
               %modifRB(cellSouci, RBsouci,nbrUser, rbUserPrisCell, tabRB, N )   
                        newRB=modifRB(q,wwRbConfli, cell{q,ta}.nbrUser, cell{q,ta}.rbUserPris  , Mat(t).tabRB , cell{q,1}.nbrRB); %N); 17/09
                      % newRB
                        %04/07/2013 ==> a voir
                        if (newRB ~= wwRbConfli)
                          %  disp('!!!!!!!!!!!!!!!!!');
                        ...
                        %    fprintf(2,'Je change %g \t avec \t %g \t vois %g \n ',wwRbConfli,newRB, voisin);
                          ...
                          %  rbU= find(cell{q,ta}.userRBPris == wwRbConfli) ;
    ...
    switch varianteStab
        case 1    
            ...
            for cc=1:V
                ...
                if (round(diffG) <= epsilon)%diffG<= epsilon) %round(diffG)==0)   % ==0   <= epsilon
...
%              
       ...
       case 4   
            ...
            for cc=1:V
                ...
                if (round(diffAPP) <= epsilon) %diffAPP <= epsilon) %round(diffAPP)==0)   % ==0   <= epsilon
                    %rajout du round 05/09
                     okStable=okStable+1;
                     celluleOKiii=[celluleOKiii, cc];
                     continuer=0;
...
%      end
          ...
          %j'ai déplacé ce test 05/09  car il était répétitif dans les 3 cas du switch  
            ...
            if(t>1) %06/09
              if ((cptStable(t) >1) &&(cptStable(t) - cptStable(t-1) == 1)) 
 ...
 if (okStable== V)
  fprintf ('---------  FIN CONVERGENCE OK   --------- reçu %d  \t msg RNTP  ---- \t %d msg CQI -- %d ms  \t duree de convergence  \n', cpt_RNTP, cpt_CQI,  tConverg);
 end
