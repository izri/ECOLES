...
N = 25;     %Number of RBs/cell  ==> 6 pr SOAPS
...
for k=1:V %etape init de mon algo
    ...
    % pris=[];
     ...
     %   plot(cell{k}.user{h}.x, cell{k}.user{h}.y, 'k+'); % pr visualiser         les users
    ...
    fprintf('%d ---- cell %d -- RB %d   --new %d \n', COUNT, q, wwRbConfli,newRB );
             ...
             switch varianteChooseRB                 
               ...
               case 3
                    ...
                    cell{q,1}.cumulDebit = cell{q,1}.cumulDebit - cell{q,t}.RB(wwRbConfli).Throughput + cell{q,t}.RB(newRB).Throughput; %20/08
...
%if      COUNT > 7   
fprintf(fiTestAAAbbbbb, '%d \t %d \t %g \t %f \t %g \t %g \n', qcell, COUNT, countITER(qcell), gainGlobal(qcell, COUNT), cell{qcell,1}.cumulDebit); %tabREWARD(q));  , Mat(t).gainCell(qcell)
% else
%  fprintf(fiTestAAAbbbbb, '%d \t %d \t %g \t %f \t %g \t %g\n', qcell, COUNT, countITER(qcell), gainGlobal(qcell, COUNT), cell{qcell,1}.cumulDebit, 0); %tabREWARD(q));   
% end
...
%             end
 ...
 for uujj=1:V
     gainGlobalTTT(uujj, t)=  gainGlobal(uujj, COUNT-1);
 end
    ...
    switch varianteStab
        case 1    
            ...
            for cc=1:V
                ...
                diffG= gainGlobalTTT(cc, t) -   gainGlobalTTT(cc, t-1);
                if (COUNT<=V)  
                     zzzz=COUNT-1;
                else
                     zzzz= mod(COUNT, V)+1;
