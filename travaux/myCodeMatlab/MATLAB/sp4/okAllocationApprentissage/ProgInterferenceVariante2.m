...
charge=0.8;
Nstabilisation=5; %pr verifier que pendant ce nbre d itérations les paramètres n ont pas changé
...
%       %27/09
     if (COUNT>1 )
         gainGlobal(q, COUNT)= gainGlobal(q, COUNT-1) + gainTotalCell(q,rb_aaA);
         ...
         for jjj=1:V
          ...
          gainGlobal(q, COUNT)=  gainTotalCell(q,rb_aaA);  
...
diffGainTotal(wwRbConfli)=-1; %27/09
...
% %                                 
                 ...
                 %%   fprintf(2, ' ----------- %d \t \t %g \t %g \t %g \t %g \n', t,   Mat(t-1).gainTotal(q,newRB), Mat(t).gain(q,newRB),Mat(t-1).gain(q,newRB), Mat(t).gainTotal(q,newRB));
                  ...
                  gainGlobal(q, COUNT)= gainGlobal(q, COUNT) - gainTotalCell(q,wwRbConfli) + gainTotalCell(q,newRB);   
...
% if   (countITER>1)
%         M(countITER).reward(q)=M(countITER-1).reward(q)+cumulGainTotal;
% else
%         M(countITER).reward(q)=0;
%         M(countITER).reward(q)=cumulGainTotal;
% end
% 

tabREWARD(q)=gainGlobal(q, COUNT); %%%%tabREWARD(q)+cumulGainTotal;
fprintf(fiTestAAAbbbbb, '%d \t %d \t %g \t %f\n', q, COUNT, countITER(q), tabREWARD(q));
