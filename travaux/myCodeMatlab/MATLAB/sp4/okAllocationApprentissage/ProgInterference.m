...
charge=0.7;
Nstabilisation=1; %pr verifier que pendant ce nbre d itérations les paramètres n ont pas changé
...
OutpuVerfi= strcat('VerfiGain.txt'); %pr le débit
...
for indiCell=1:V
 ...
 %  fprintf('--------------- t=%g\t\t q=%g\t\t RB=%g \t \t SINR =%f  \n',t,indiCell, rb_aA, cell{indiCell,t}.RB(rb_aA).SINR);
      ...
      % if (length(intersect(T{sauvegardddd}.cellO{indiCell},rb_aA )) > 0)  
         ...
         %  disp('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');  
         Mat(t).gainTotal(indiCell,rb_aA) = Mat(t).gainTotal(indiCell,rb_aA) + Mat(t).gain(indiCell,rb_aA);
      ...
      gainTotalCell(indiCell,rb_aA)=Mat(t).gainTotal(indiCell,rb_aA); %gainTotalCell(indiCell,rb_aA)+
...
%faire une fonction pr calculer le SINR ==> calculSINR
 ...
 %04/09 ==> j'ai enlevé dans les conditions de la boucle while le test
 %"(t<=T_max)  &&" pr voir à quel endroit on atteint la stabilité
while ((t<=T_max) && (okStable~=V) )  %&& (continuer ~= 0) && (okStable~=V) &&
fprintf(2,' t=%g \t \t cptStable=%g \t \t okStable= %g  \n',t, cptStable, okStable);
boolll=-1;
boolTest=-1;
      ...
      %30/08
                    ...
                    if(t>1)   
                      if (length(intersect(T{sauv}.Cel{q},wwRbConfli )) > 0)    
                       ...
                       Mat(t).gain(q,wwRbConfli)= Mat(t).apprentissageRB(q,wwRbConfli) -  Mat(t-1).apprentissageRB(q,wwRbConfli);
                       Mat(t).gainTotal(q,wwRbConfli) = Mat(t-1).gainTotal(q,wwRbConfli) + Mat(t).gain(q,wwRbConfli);
                   ...
                   gainTotalCell(q,wwRbConfli)=Mat(t).gainTotal(q,wwRbConfli); %gainTotalCell(q,wwRbConfli)+
      ...
      %30/08
                            ...
                            if(t>1) 
                               ...
                               if (length(intersect(T{sauv}.Cel{q},newRB )) > 0)    
                                   Mat(t).gain(q,newRB)= Mat(t).apprentissageRB(q,newRB) - Mat(sauv).apprentissageRB(q,newRB); %cell{q,t}.probCollision*apprentissageRB(q,ww);
                                 %  disp('???????????????');  
                            ...
                            gainTotalCell(q,newRB)=Mat(t).gainTotal(q,newRB); %gainTotalCell(q,newRB)+
...
%        else 
           %30/08
         Mat(t).gain(q,rb_A)= Mat(t).apprentissageRB(q,rb_A) - Mat(t-1).apprentissageRB(q,rb_A);
      ...
      %  end
         ...
         Mat(t).gainCell(q)=0;
         Mat(t).gainTotal(q,rb_A) =  Mat(t).gain(q,rb_A);
      end
...
%        else
         ...
         Mat(t).gain(q,rb_aaA)= Mat(t).apprentissageRB(q,rb_aaA) - Mat(t-1).apprentissageRB(q,rb_aaA);
...
if t>=2  
    switch varianteStab
        case 1    
            ...
            diffG=0;
            for cc=1:V
                diffG=Mat(t).gainCell(cc)-Mat(t-1).gainCell(cc); %abs
                if (diffG<= epsilon)   % ==0
                     okStable=okStable+1;
                     continuer=0;
                else
            ...
            %voir par rapport aux SINR des RBs utilisés
             ...
             for xxC=1:V
                for allRBx=1:cell{xxC,ta}.nbrUser   %length(cell{xx,ta}.rbUserPris)
                    ...
                    indexRB=cell{xxC,ta}.rbUserPris(allRBx);
                    aa=t-1; 
                    diffSINR = cell{xxC,t}.RB(indexRB).SINR - cell{xxC,aa}.RB(indexRB).SINR; %abs 
                  %  cell{q,t}.RB(newRB).SINR
                    %Mat(t).gainCell(cc)-Mat(t-1).gainCell(cc); 
                    if (diffSINR<= epsilon) %==0
